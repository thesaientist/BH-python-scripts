...
# low - poor stacking)
def reflStack(angle, measDepth, zData, rStep, dataNInterp):
...
def bestAzimuth(measDepth, zData, rStep, dataNInterp):
	...
	for i in range(trialAngles.shape[0]):
		...
		if depthRow + 15 > zData.shape[0]-1 or depthRow - 15 < 0:
			break
		stdAngles[i], originalTraceData, shiftedTraceData, sumReflData = reflStack(trialAngles[i], measDepth, zData, rStep, dataNInterp)
		#DEBUG: --
...
for i in range(zDataDFN.shape[0]):
	azimuthDFN[i] = bestAzimuth(zDataDFN[i], zData, rStep, dataNInterp)
...
plt.title('ADNOC DE03H DFN Azimuth Estimation (Negative Trace Only)')
plt.show()
