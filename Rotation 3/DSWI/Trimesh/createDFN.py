...
dfn = DFNlib.DFN()
for d in np.array(las.getData()).T[::100]:
    md, hzup, hzdn, vtdn, vtup, intn = d
    x0, y0, z0, azi = interpolate_from_path(md, path)
    dx = hzup
    dy = hzdn
    p1 = DFNlib.Point(coords=(x0, y0, z0))
    p2 = DFNlib.Point(coords=(x0, y0, z0+vtup))
    p3 = DFNlib.Point(coords=(x0, y0, z0+vtdn))
    p4 = DFNlib.Point(coords=(x0+dx, y0+dy, z0))
    p5 = DFNlib.Point(coords=(x0-dx, y0-dy, z0))
